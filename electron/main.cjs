...
ipcMain.handle('local-wokwi:set-native-sensor', async (_event, request) => runtime.setNativeSensor(request));
ipcMain.handle('local-wokwi:send-bus-transaction', async (_event, request) => runtime.sendBusTransaction(request));
